...
// State with default values (stored in IndexedDB)
const defaultState = {
  candidates: [],
  teams: [],
  auction: {
...
async function loadState() {
  try {
    ...
    return new Promise((resolve) => {
        ...
        if (event.target.result?.data) {
          const newState = {
            ...
            auction: {
              ...
              ...(event.target.result.data.auction || {})
            }
          };
          if (newState.auction.stateVersion > lastStateVersion) {
            state = newState;
            lastStateVersion = newState.auction.stateVersion;
      ...
      request.onerror = () => {
        console.warn('Error loading state, using default values');
        resolve();
      };
    });
  } catch (e) {
    console.error('Database error:', e);
    alert('Error loading data. Please reset the system if the problem persists.');
  }
}
...
async function saveState(silent = true) {
  try {
    ...
    await new Promise((resolve, reject) => {
      store.put({ id: 'appState', data: state });
      tx.oncomplete = () => {
    ...
    complete: async function(results) {
      ...
      rows.forEach((row, index) => {
        if (row.length >= 3) { // Expect at least Name, Admission Number, Class
          ...
          const classNumber = parseInt(trimmedClass);
  ...
  if (tbody) {
    ...
    availableCandidates
      .filter(c => c.name.toLowerCase().includes(search) || 
                 c.class.toString().includes(search))
      .forEach(candidate => {
        const tr = document.createElement('tr');
        let requestBtn = '';
...
async function nextTurn() {
  ...
  if (remaining === 0) {
    let nextClass = parseInt(state.auction.currentClass) + 1;
...
function startPolling() {
  ...
  pollInterval = setInterval(async () => {
    ...
    console.log('Polled state update');
  }, POLL_INTERVAL);
}
